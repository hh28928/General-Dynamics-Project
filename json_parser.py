...
for a in data:
    ...
    for lsm in data[a][1]:
        ...
        if type(data["devices"][1][lsm]) is float:
            json_metrics += "{\"metric\": \"d1.lsm." + lsm + "\",\"timestamp\": %.0f" %time.time()
        ...
        for value in data["devices"][1][lsm]:
            temp = value[:-2]
            temp_d = value[-1:]
            json_metrics += "{\"metric\": \"d1.lsm." + temp+"."+temp_d + "\",\"timestamp\": %.0f" %time.time()
            json_metrics += ",\"value\": %.18f" % data["devices"][1][lsm][value]
            json_metrics += ",\"tags\": {\"sensor\": \"LSM9DS1\",\"device\": \"1\"}},"
    for bmp in data[a][2]:
        if bmp == "device":
            continue
        json_metrics += "{\"metric\": \"d1.bmp." + bmp + "\",\"timestamp\": %.0f" %time.time()
        json_metrics += ",\"value\": %.18f" % data["devices"][2][bmp]
        json_metrics += ",\"tags\": {\"sensor\": \"BMP280\",\"device\": \"1\"}}," 
    for dht in data[a][3]:
        if dht == "device":
            continue
        json_metrics += "{\"metric\": \"d1.dht." + dht + "\",\"timestamp\": %.0f" %time.time()
